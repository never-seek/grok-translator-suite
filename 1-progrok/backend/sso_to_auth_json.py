...
def _clean_sso_cookie(raw: str) -> str:
    val = (raw or "").strip()
    if not val:
        return val
    # Accept a whole Cookie header line, sso=..., sso-rw=..., or a bare JWT.
    for part in val.split(";"):
        name, sep, value = part.strip().partition("=")
        if sep and name.strip() in {"sso", "sso-rw"} and value.strip():
            return value.strip()
    if val.startswith("sso=") or val.startswith("sso-rw="):
...
def sso_to_token(sso_cookie: str, *, quiet: bool = False) -> dict | None:
    ...
    sso_cookie = _clean_sso_cookie(sso_cookie)
    s = requests.Session()
    for _domain in (".x.ai", "accounts.x.ai", ".accounts.x.ai", "auth.x.ai"):
        try:
            s.cookies.set("sso", sso_cookie, domain=_domain, path="/")
            s.cookies.set("sso-rw", sso_cookie, domain=_domain, path="/")
        except Exception:
            pass
    timeout = _http_timeout()
