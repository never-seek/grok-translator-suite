...
def cfmail_create_mailbox(
    *,
    ...
    # Local catch-all mailbox workers do not need to pre-create addresses.
    # Some deployments expose only one default domain from /api/new_address and
    # ignore the requested domain; for domain-rotation tests we must still use
    # the configured catch-all domain.  When the create response comes back on a
    # different domain, mint an address token for the requested address and use
    # that instead.  The Worker email() handler stores any routed recipient, and
    # /api/token in this local worker issues a token for an arbitrary address.
    try:
        requested_domain = (dom or "").strip().lower()
        returned_address = str(address or "").strip().lower()
        ...
        if requested_domain and returned_domain and returned_domain != requested_domain:
            ...
            with httpx.Client(timeout=20.0) as client:
                tresp = client.post(
                    ...
                    headers=_cfmail_headers(api_key=key, site_password=site_password, content_type=True),
                )
            if tresp.status_code < 400:
                tdata = tresp.json() if tresp.content else {}
                tbody = tdata.get("data") if isinstance(tdata, dict) and "data" in tdata else tdata
                if isinstance(tbody, dict):
                    tjwt = tbody.get("jwt") or tbody.get("token") or tbody.get("credential") or ""
                    if tjwt:
                        body = dict(body)
                        body["domain_override"] = {"from": returned_domain, "to": requested_domain}
                        address = desired_address
                        jwt = str(tjwt)
                        address_id = desired_address
    except Exception:
        # Keep provider compatibility: if token mint fails, continue with the
        # original provider response and let the caller surface any real error.
        pass
    if not address or "@" not in str(address):
